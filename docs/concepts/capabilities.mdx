...
`AbstractAgent` declares an optional `getCapabilities()` method that returns a
typed snapshot of everything the agent currently supports. It is implemented by
agent subclasses that know their own capabilities — the LangGraph and ADK
integrations, or your own — not by the bare `HttpAgent`, which is a transport
and has no discovery endpoint to ask. The optional call below returns
`undefined` for an agent that does not implement it:
