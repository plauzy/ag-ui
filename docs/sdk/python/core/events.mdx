...
| `input_tokens`             | `Optional[int]` | All input tokens, cache reads and writes included               |
| `output_tokens`            | `Optional[int]` | All output tokens, reasoning included                           |
| `total_tokens`             | `Optional[int]` | `input_tokens` plus `output_tokens`                             |
| `reasoning_tokens`         | `Optional[int]` | Output tokens spent on reasoning; part of `output_tokens`       |
| `cached_input_tokens`      | `Optional[int]` | Input tokens read from a prompt cache; part of `input_tokens`   |
| `cache_write_input_tokens` | `Optional[int]` | Input tokens written to a prompt cache; part of `input_tokens`  |
...
as an optional `subagent_run_id` on most other event types.

`subagent_run_id` identifies **one invocation**, not a reusable subagent
definition — the same subagent run twice yields two different values. See
[Subagents](/concepts/subagents) for the full model.

Field names are snake_case in Python and serialize to camelCase on the wire, so
`subagent_run_id` appears as `subagentRunId` in JSON.

### SubagentStartedEvent

`from ag_ui.core import SubagentStartedEvent`
...
| Property                 | Type  | Description                                   |
| ------------------------ | ----- | --------------------------------------------- |
| `subagent_run_id`        | `str` | Opaque identifier for this invocation         |
| `name`                   | `str` | Declared subagent name or type, for display   |
| `description`            | `str` | Optional description                          |
| `parent_subagent_run_id` | `str` | Optional enclosing subagent, when nesting     |
| `parent_tool_call_id`    | `str` | Optional tool call that spawned this subagent |
| `parent_message_id`      | `str` | Optional message holding that tool call       |
...
| Property          | Type  | Description                                            |
| ----------------- | ----- | ------------------------------------------------------ |
| `subagent_run_id` | `str` | Matches the id from `SubagentStartedEvent`               |
| `result`          | `Any` | Optional payload, mirroring `RunFinishedEvent.result`   |
| `outcome`         | `Optional[SubagentFinishedOutcome]` | Discriminated union of `SubagentFinishedSuccessOutcome` and `SubagentFinishedSuspendedOutcome` (with optional `interrupt_ids`); `None` means success (the legacy reading). Suspended says the subagent is checkpointed awaiting outside input; `interrupt_ids` names the run-level interrupts whose answers resume it. |

### SubagentErrorEvent

`from ag_ui.core import SubagentErrorEvent`
...
Most event types accept an optional `subagent_run_id`. An event without it
belongs to the parent agent, so a stream that never sets the field behaves
exactly as it did before subagents existed.
